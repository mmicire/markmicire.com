...
def nl_to_br_outside_tags(t: str) -> str:
    """Replace newlines with <br> only when we're NOT inside an HTML tag."""
    out = []
    in_tag = False
    quote = None
    for ch in t:
        if in_tag:
            if quote:
                ...
                if ch in ("'", '"'):
                    ...
                    # inside a tag, normalize newline to a single space to avoid breaking attributes
                    out.append(" ")
                else:
                    out.append(ch)
        else:
...
def auto_html(text: str) -> str:
    ...
    - Mixed/HTML present: DO NOT wrap in <p>, but convert bare \n to <br> outside tags only.
    """
    t = (text or "")
    if not t.strip():
        return ""
    ...
    if not has_tags:
        # Paragraphs by blank lines; <br> for single newlines
        blocks = re.split(r"\n\s*\n", t.strip())
        return "\n".join("<p>{}</p>".format(b.strip().replace("\n", "<br>")) for b in blocks)
    else:
        # Keep existing HTML, make plain newlines visible but never inside tags
...
<style>
  :root {
    /* Column widths */
    --three-left: 100px; /* label */
    --three-mid: 380px;  /* main */
